...
void set_led_off(void)
{
	...
	tmp |= (amberled + redled);	//;GPIO71,72 set 11b  //0x06
...
void set_led_red(void)
{
	...
	tmp = read_sio_reg(LDN7, 0xe1) & ~(amberled + redled);
	tmp |= redled;	//;GPIO71,72 set 10b     //0x04
	write_sio_reg(LDN7, 0xe1, tmp);
	printf("%lx\n",tmp);	
...
void set_led_amber(void)
{
	...
	tmp = read_sio_reg(LDN7, 0xe1) & ~(amberled + redled);
	tmp |= amberled;	//;GPIO71,72 set 01b  //0x02
	write_sio_reg(LDN7, 0xe1, tmp);
	printf("%lx\n",tmp);	
